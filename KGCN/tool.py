...
@logger('Building an undirected graph based on the knowledge graph structure')
def construct_undirected_kg(kg: List[Tuple[int, int, int]]) -> Dict[int, List[Tuple[int, int]]]:
    ...
    for head_id, relation_id, tail_id in kg:
        ...
        kg_dict[tail_id].append((relation_id, head_id))  # Treat the knowledge graph as an undirected graph
    return kg_dict


@logger('Building an adjacency list from an undirected knowledge graph，', ('n_entity', 'neighbor_size'))
