...
class GATNEModel(Model):
    #self.feature_dic Whether to use GATNE-I or T, whether to add node features 
    def __init__(self, num_nodes, embedding_size, embedding_u_size, edge_type_count, dim_a,feature_dic,vocab,num_sampled,neighbor_samples):
        ...
        if self.feature_dic is not None:
            ...
            #u_embed_trans seems to be the type of edge combined with the embed
    ...
    def call(self,train_inputs, train_labels,train_types, node_neigh):
        ...
        if self.feature_dic is not None:
            ...
            # Four-dimensional vector [number of nodes in training set, number of edge types, fixed sequence length, initial node feature dimension]
            #Concat the kth-order neighbors, different types of edge nodes, and the corresponding edge embeddings to concat:
            ...
            #tf.transpose transpose, tf.reduce_mean average
            #Averaging node_embed_tmp is node_type_embed
        ...
        #tf.nn.nec_loss() Use the negative sampling function in skip_gram to calculate the loss function of each batch data
